...
<div class="container">
  ...
  <hr/>
  <div>
  	앞의 정의된 라이브러리를 사용할 경우에는 jsp지시자 'taglib'를 이용하여 먼저 선언 후 사용한다.<br/>
  	주소(download) : https://tomcat.apache.org/taglibs/standard/
  </div>
  <hr/>
  <h2>Core 라이브러리 사용 예</h2>
  <pre>
  	<h4 class="mt-5">용도 : 변수제어(선언, 값 할당, 객체, 출력, 제거), 제어문(조건문, 반복문)</h4>
  ...
  su1 값을 출력1 <c:out value="${su1}"></c:out><br/>
  su1 값을 출력2 ${su1}<br/>
  ...
  <h4 class="mt-5">JSTL 제어문(조건문(if), 반복문(forEach)) - 조건의 비교는 EL을 이용한다.</h4>
  ...
  <div>3. su3 > su4 : <c:if test="${su3 + '0' > su4 + '0'}"> su3가 su4다 크다.</c:if></div>
  <div>4. su3 <= su4 : <c:if test="${su3 + '0' <= su4 + '0'}"> su3가 su4다 작거나 같다.</c:if></div>
  <div>5. su3 < su4 : <c:if test="${su3 <= su4}"> su4가 su3다 크다.</c:if></div>
  <div>6. su3 >= su4 : <c:if test="${su3 >= su4}"> su4가 su3다 작거나 같다.</c:if></div>
  <div>3_1. su3 gt su4 : <c:if test="${su3 gt su4}"> su3가 su4다 크다.</c:if></div>
  <div>4_1. su3 le su4 : <c:if test="${su3 le su4}"> su3가 su4다 작거나 같다.</c:if></div>
  <div>5_1. su3 lt su4 : <c:if test="${su3 lt su4}"> su4가 su3다 크다.</c:if></div>
  <div>6_1. su3 ge su4 : <c:if test="${su3 ge su4}"> su4가 su3다 작거나 같다.</c:if></div>
  <h4 class="mt-5">다중조건비교 : choose ~ when ~ otherwise</h4>
  <pre>
  	사용법 : 
  	< c : choose>
  		< c : when test="조건식1"> 처리할 내용1 < / c : when>
  		< c : when test="조건식2"> 처리할 내용1 < / c : when>
  		< c : when test="조건식3"> 처리할 내용1 < / c : when>
  		~~ ~~ ~~
  ...
  <div>점수 85점인 학생의 학점은? 학점 :
  	...
  	<c:choose>
  		...
  		<c:when test="${jum >= 50}">D</c:when>
  		<c:otherwise>F</c:otherwise>
  	</c:choose>
  </div>
  <hr/>
  <h4 class="mt-5">반복문(forEach)</h4>
  <%for(int i = 1; i <= 10; i++) { %>
  	값1 : <%=i %> /
  <% } %>
  <br/>
  ...
  <br/>
  <c:set var="cards2" value="신한/국민/BC/LG/현대/삼성" />
  카드 : ${cards2}<br/>
  <c:forTokens var="card" items="${cards2}" delims="/">
  	${card} :
  </c:forTokens>
  <br/>
  <h4 class="mt-5">상태변수 사용(varStatus)</h4>
  <pre>
  ...
  <br/>
  <h4>images 저장된 그림 5장을 출력하시오.</h4>
  <c:forEach var="i" begin="1" end="5">
  	그림1 : <img src="${pageContext.request.contextPath}/images/js/${i}.jpg" width="150px" />
  </c:forEach>
  <br/>
  <c:set var="img" value="1/2/3/4/5/" />
  <c:forTokens var="i" items="${img}" delims="/" varStatus="st">
  	그림2 : <img src="${pageContext.request.contextPath}/images/js/${i}.jpg" width="150px" />
...
%>
  ...
  <hr/>
  <form name="myform" method="post" action="<%=request.getContextPath() %>/j0807/JSTLtest1_2">
  	<div class="mb-3">성명
	  	<input type="text" name="name" id="name" value="이동준" class="form-control mb-3" autofocus required />
	  	</div>
	  	<div class="mb-3">나이
 	...
 	<div class="col"><input type="submit" value="전송(submit)" onclick="fCheck()" class="btn btn-success"/></div> 	
  </form>
  	<div id="demo">
  		성명 : ${vo.name} <br/>
  		나이 : ${vo.age}<br/>
